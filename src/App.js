...
export default function App() {
  ...
  const gameLoop = () => {
    ...
    if (varieties[0].actions[0].day === day) {
      addRaft();
    }
  };
  ...
  // turning this on for the first time
  const start = () => {
    if (timer === false) {
  ...
  const addNewVegetable = (index) => {
    let germCopy = [...germination];
    germCopy.push(new Tray(varieties[index]));
  ...
  return (
    <div className="App">
      ...
      <div className="container mt-3 mb-5">
        <div className="row">
          ...
          <div className="col-4 text-end">
            <button
              disabled={timer}
              className={!timer ? "btn btn-success" : "btn btn-secondary"}
              onClick={start}
            >
              Start
            </button>
      ...
      <div className="container mb-4">
        <div className="row">
          <div className="col">
            {varieties.map((ele, index) => {
              return (
                <button
                  onClick={() => addNewVegetable(index)}
                  key={index}
                  className={ele.buttonStyle + "m-1"}
                >
                  {ele.name}
                </button>
              );
            ...
            >
              <div className="row">
                {germination.map((tray, ti) => (
                  <div
                    className={tray.variety.borderStyle + "m-2"}
                    key={ti}
                    style={{
                      minWidth: "70px",
                      maxWidth: "70px",
